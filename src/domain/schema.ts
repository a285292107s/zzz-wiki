 ...
 * 结构变更先改 schema，再同步类型。
 *
 * ── .catchall(z.unknown()) 设计取舍 ──
 *
 * 全部 schema 的末尾都追加了 .catchall(z.unknown())，其取舍如下：
 *
 *   好处（为什么加）：
 *   - 前向兼容：hakushin raw 持续新增字段（strategy / fairy_recommend / skill_list …），
 *     schema 无需逐字段同步，verify-data 不会因未知键而误报失败。
 *   - 降级安全：个别数据文件的字段缺失/类型异常时，已知字段仍能通过校验，
 *     不会因一个字段坏掉而整份标记为无效。
 *
 *   代价（知道即可）：
 *   - 未知字段的增删不会触发校验失败——契约的约束力集中在已声明的字段上。
 *   - 详情 schema 中 skill / talent / passive / skin 等复杂嵌套字段使用了
 *     z.record(z.unknown())，即"接受任意形状"。这些字段的实际结构由前端
 *     domain/sections.ts 的运行时解析逻辑承担，schema 不做深度约束。
 *     若未来需要加强，应逐字段分析真实数据形状后收紧，不可凭猜测改动。
 *
 *   当前 verify-data 的实际效力 = "已知字段存在且类型正确 + 未知字段不阻断"。
 * ============================================================ */
